...
def _get_rmat(_sfx1c, x=None):
    ...
    s1 = s + reduce(numpy.dot, (x.conj().T, t, x)) * (0.5 / c**2)
    return _get_r(s, s1)
...
def get_X_R_dm_LL_LS_SS(_mol, _sfX2C, _cmoao, _dm1):
    ...
    if _sfX2C.with_x2c.xuncontract and contr_coeff is not None:
        dm1_AO_uncontracted = reduce(numpy.dot, (contr_coeff, dm1_AO, contr_coeff.T))
    else:
        dm1_AO_uncontracted = dm1_AO.copy()
    dm1_LL = reduce(numpy.dot, (R0, dm1_AO_uncontracted, R0.T))  # R0 is real
...
def fetch_soc2e_SOMF_BP(_mol, _cmoao, _dm1, _SOMF=True):
    ...
    rdm1_atom = reduce(numpy.dot, (cmoao, _dm1, cmoao.T))  # I am so stupid here !
    ...
    if _SOMF:
        vj, vk, vk2 = pyscf.scf.jk.get_jk(
            _mol,
            [rdm1_atom, rdm1_atom, rdm1_atom],
    ...
    # Check AntiSymmetry
    for x in hSOMF_atom:
        print("Check Antisymmetry in fetch_soc2e_SOMF_BP %e" % (Check_AntiSymmetry(x)))
...
def _Get_Wsd(_V):
    ...
    vz = _V[2]
    return numpy.bmat(
        [[vz, vx + vy * complex(0, -1)], [vx + vy * complex(0, 1), -vz]]
    ) * complex(0, 1)
...
def fetch_X2C_soDKH1(
    _mol, _sfX2C, _cmoao, _dm1, _test=False, _get_1e=True, _get_2e=True
):
    # with uncontracted orbitals, more precise!
    xmol, contr_coeff = _sfX2C.with_x2c.get_xmol(_mol)
    # get rdm in AO base
    ...
    if _get_1e:
        ...
        E_plus_1_sf, E_plus_1_sd = _Get_Wsf_Wxyz(
            E_plus_1
        )  # E_plus_1_sf should be zero matrix !
        for x in E_plus_1_sd:
            print(
                "Check Antisymmetry in fetch_X2C_soDKH1 get 1e %e"
    ...
    if _get_2e:
        ...
        g_LL_1, g_LS_1, g_LS_2, g_SS_1, g_SS_2, g_SS_3 = pyscf.scf.jk.get_jk(
            xmol,
            [dm1_SS, dm1_LS, dm1_LS, dm1_LL, dm1_LL, dm1_LL],
            [
                "abcd,ac->bd",
                "abcd,bc->ad",
                "abcd,ac->bd",
        ...
        # contract X
        g_LS = numpy.asarray([reduce(numpy.dot, (x, X0)) for x in g_LS])
        g_SL = numpy.asarray([reduce(numpy.dot, (X0.T, x)) for x in g_SL])
        g_SS = numpy.asarray([reduce(numpy.dot, (X0.T, x, X0)) for x in g_SS])
        # construct g
        g = g_SS + g_SL + g_LS + g_LL

        for x in g:
            print(
                "Check Antisymmetry in fetch_X2C_soDKH1 get 2e %e"
                % (Check_AntiSymmetry(x))
            )
