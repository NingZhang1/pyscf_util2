...
def dump_heff_casci(_mol, _mcscf, _core_coeff, _mocoeff, _filename="FCIDUMP"):
    ...
    if isinstance(_mcscf._scf._eri, np.ndarray):
        int2e_full = pyscf.ao2mo.full(
            eri_or_mol=_mcscf._scf._eri, mo_coeff=mocoeff, compact=True
        )
    else:
        int2e_full = pyscf.ao2mo.full(
    ...
    if _filename == None:
        return (
            ...
            energy_core,
            _mocoeff.shape[1],
            _mol.nelectron - 2 * _core_coeff.shape[1],
            OrbSymID,
        )
    else:
        tools.fcidump.from_integrals(
            ...
            nelec=_mol.nelectron - 2 * _core_coeff.shape[1],  # Useless
            tol=1e-10,
            orbsym=OrbSymID,
        )
