...
def symmetrize_rdm1(orbsym_ID, rdm1, begin_orb, end_orb, xy_basis=False):
    ...
    if xy_basis:
        rdm1_tmp = tranform_rdm1_xy_2_adapted(orbsym_ID, rdm1_tmp, begin_orb, end_orb)
        ...
        for j in range(end_orb - begin_orb):
            if orbsym_ID[i + begin_orb] == orbsym_ID[j + begin_orb]:
                ...
                if abs(rdm1_tmp[i, j]) > 1e-6:
                    print("Warning %d %d rdm1 %.4e too large" % (i, j, rdm1_tmp[i, j]))
...
def symmetrize_rdm2(orbsym_ID, rdm2, begin_orb, end_orb, xy_basis=False):
    ...
    if xy_basis:
        rdm2_tmp = tranform_rdm2_adapted_2_xy(orbsym_ID, rdm2_tmp_, begin_orb, end_orb)
    else:
        rdm2_tmp = rdm2_tmp_
...
def FCIDUMP_Coov(mol, my_scf, filename):
    orbsym_ID, _ = get_orbsym(mol, my_scf.mo_coeff)
    basis_trans, _ = get_symmetry_adapted_basis_Coov(mol, my_scf.mo_coeff)

    # construct h1e and h2e

    h1e = reduce(numpy.dot, (my_scf.mo_coeff.T, my_scf.get_hcore(), my_scf.mo_coeff))

    h1e_adapted = reduce(numpy.dot, (basis_trans.H, h1e, basis_trans))

    int2e_full = pyscf.ao2mo.full(
        eri_or_mol=mol, mo_coeff=my_scf.mo_coeff, aosym="1"
